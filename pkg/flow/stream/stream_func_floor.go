...
func NewFFIFloor() itypes.FlowCall {
	return ffi.NewFFI(ffiFloor{})
}

func (f ffiFloor) Call(i itypes.Interpreter) (itypes.Object, error) {
	if f.Input.Int != nil {
		return f.Input.Int, nil
	} else if f.Input.Double != nil {
		return primitive.NewObjectDouble(math.Floor(f.Input.Double.Value)), nil
	} else {
		return NewStreamFuncFloor(prototypeStreamDouble, f.Input.Stream), nil
	}
}
